...
from typing import Callable, TypeVar, ParamSpec, Concatenate, Awaitable, Hashable
...
def method(
    target_cls: type[T],
    group_cls: type[Group[T]],
) -> Callable[..., Callable[[Callable[Concatenate[T, P], Awaitable[R]]], Callable[P, Callable[[T], Awaitable[R]]]]]:
    def deco(
        prefix: str | None = None,
    ) -> Callable[[Callable[Concatenate[T, P], Awaitable[R]]], Callable[P, Callable[[T], Awaitable[R]]]]:
        def _deco(
            func: Callable[Concatenate[T, P], Awaitable[R]],
        ) -> Callable[P, Callable[[T], Awaitable[R]]]:
            def _func(*args: P.args, **kwargs: P.kwargs) -> Callable[[T], Awaitable[R]]:
                async def _partial(target: T) -> R:
            ...
            setattr(target_cls, func.__name__ if prefix is None else f"{prefix}_{func.__name__}", _target_method)
            ...
            setattr(group_cls, func.__name__ if prefix is None else f"{prefix}_{func.__name__}", _group_method)
